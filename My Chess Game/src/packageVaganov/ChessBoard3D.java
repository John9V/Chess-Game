...
/** Creates tiles, puts pieces on them and stores them in a 3d array. */
public class ChessBoard3D extends Application implements Board{
    ...
    public ChessBoard3D() {
        tiles = new Tile[sideLength][sideLength][3];
    ...
    /** Checks the occupancy of a tile given its x , y, and z. */
    public boolean isTileOccupied(int x, int y, int z) {
        if (x > 7 || y > 7 || x < 0 || y < 0) {
            return true;
        }
        if (tiles[y][x][z].isOccupied()) {
            return true;
        }
        return false;
    }
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        Scene scene = new Scene(root, 1315, 415, Color.BLACK);
                ...
                for (int j = 0; j < sideLength; j++) {
                    ...
                    if ((i % 2 == 0 && j % 2 == 0)
                        ...
                        tiles[i][j][k] = new Tile(
                                this, i, j, k, whiteTileColor, control);
                    } else {
                        tiles[i][j][k] = new Tile(
                                this, i, j, k, blackTileColor, control);
                    }
                    selectedTile = tiles[i][j][k];
                    selectedTile.draw(g);
                    setUpPiece(i, j, k);
                }
            }
